...
function useMockDAO() {
  ...
  const [treasury, setTreasury] = useState(24580);
  ...
  const vote = (id, forIt, wallet) => {
    ...
    setProposals(prev => prev.map(pr => 
      pr.id === id ? { 
        ...pr, 
        votesFor: forIt ? pr.votesFor + 1 : pr.votesFor, 
        votesAgainst: !forIt ? pr.votesAgainst + 1 : pr.votesAgainst 
      } : pr
    ));
  };
  ...
  const approveChange = (id, walletAddress) => {
    ...
    if (change && change.votesFor > authorizedCollaborators.length / 2) {
      if (walletAddress === "0xe8c8578E2889bB7bFB2869A0DBDA0c65D9f4F8A9") {
        ...
        if (change.type === "revenueSplit") {
          // This would update creators in parent component
        }
